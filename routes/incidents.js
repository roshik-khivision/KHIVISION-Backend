...
// Unprotected route for the local Python CCTV script
router.post('/cctv', geocodingLimiter, gridfsUpload, async (req, res, next) => {
  try {
    const User = require('../models/User');
    // Find any user to act as the reporter
    let user = await User.findOne({ role: 'citizen' });
    if (!user) user = await User.findOne();
    if (!user) return res.status(400).json({ success: false, message: 'No users found in database' });
