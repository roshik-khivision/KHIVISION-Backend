...
exports.updateDetails = async (req, res, next) => {
  try {
    ...
    // This prevents setting name/email/phone to undefined when only
    // fcmToken is sent from the mobile app.
    ...
    if (req.body.fcmToken !== undefined) fieldsToUpdate.fcmToken = req.body.fcmToken;

    console.log(`📝 updateDetails called for user ${req.user.id}. Fields to update:`, Object.keys(fieldsToUpdate));
    if (fieldsToUpdate.fcmToken) {
      console.log(`✅ Received fcmToken: ${fieldsToUpdate.fcmToken.substring(0, 20)}...`);
    }

    const updateQuery = { $set: fieldsToUpdate };
    if (fieldsToUpdate.fcmToken) {
      updateQuery.$addToSet = { fcmTokens: fieldsToUpdate.fcmToken };
    }
...
exports.logout = async (req, res, next) => {
  try {
    // Clear FCM token
    const updateQuery = { $unset: { fcmToken: 1 } };
    if (req.body.fcmToken) {
      updateQuery.$pull = { fcmTokens: req.body.fcmToken };
    }
