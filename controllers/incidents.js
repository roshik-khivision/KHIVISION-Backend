...
const hospitalUser = await User.findOne({
  ...
  role: 'hospital'
    ...
    // 🔔 Send FCM push notification to hospital
    try {
      const AlertService = require('../services/alertService');
      const hospitalUserWithToken = await User.findById(hospitalUser._id).select('fcmToken fcmTokens');
      if (hospitalUserWithToken && (hospitalUserWithToken.fcmToken || (hospitalUserWithToken.fcmTokens && hospitalUserWithToken.fcmTokens.length > 0))) {
        await AlertService.sendPushToUser(hospitalUserWithToken, {
          title: '🚑 New Patient Incoming',
          body: `Driver ${req.user.name} is bringing a patient. ETA: ${eta} min. Priority: ${incident.priority}`,
          data: {
            type: 'hospital_request',
        ...
        console.log(`✅ FCM push sent to hospital: ${hospitalUser.hospital || hospitalUser.name}`);
      } else {
        console.log(`⚠️ Hospital user has no FCM token — skipping push`);
      }
    } catch (fcmErr) {
      console.error('⚠️ FCM push to hospital failed (non-fatal):', fcmErr.message);
    }
...
console.log('   match:', incident?.hospitalRequest?.hospitalId?.toString() === hospitalId);
    ...
    if (!incident.hospitalRequest || 
        (incident.hospitalRequest.hospitalId?.toString() !== hospitalId && 
         incident.hospitalRequest.hospitalName !== (req.user.hospital || req.user.name))) {
      return res.status(403).json({ 
        success: false, 
  ...
  try {
    ...
    const pendingIncidents = await Incident.find({
      $or: [
        ...
        { 'hospitalRequest.hospitalName': hospitalName }
      ],
...
exports.updateHospitalStatus = async (req, res, next) => {
  try {
    ...
    // Automatically accept any pending hospital requests from the driver if admitted
    if (status === 'admitted' && incident.hospitalRequest && incident.hospitalRequest.status === 'pending') {
      incident.hospitalRequest.status = 'accepted';
      incident.hospitalRequest.respondedAt = new Date();
      incident.hospitalRequest.responseReason = 'Automatically accepted upon admission';
      console.log('✅ Automatically accepted pending hospital request due to admission.');
    }
...
exports.createIncident = async (req, res, next) => {
  try {
    ...
    if (bestAIScore >= 85) {
      initialStatus = 'approved'; // Bypass admin review, trigger auto-dispatch
      initialPriority = 'critical';
      verificationNeeded = false;
      console.log(`🔥 AI: Auto-Approved (Score: ${bestAIScore} >= 85) - Direct driver dispatch`);
    } else if (bestAIScore < 50) {
      initialStatus = 'rejected';
      console.log(`🚫 AI: Auto-Rejected (Score: ${bestAIScore} < 50) - Flagged for admin verification`);
    } else {
      initialStatus = 'pending';
      console.log(`✅ AI: Verified (Score: ${bestAIScore} >= 50 and < 85) - Sent to admin for review`);
    }
...
// @access  Private (Driver)
  ...
  try {
    ...
    if (req.io) {
      req.io.emit('incidentUpdated', incident);
      req.io.to(`driver_${driverId}`).emit('incidentUpdated', incident);
      req.io.to('departments').emit('incidentUpdated', incident);
      req.io.to('admins').emit('incidentUpdated', incident);
...
exports._performReassignment = _performReassignment;
...
exports.getDriverIncidents = async (req, res, next) => {
  try {
    ...
    // Just fetch all incidents assigned to this driver
    const incidents = await Incident.find({
      'assignedTo.driver': driverId
    })
    .populate('reportedBy', 'name email phone')
