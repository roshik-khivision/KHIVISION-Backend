...
class DriverMatchingQueue {
  ...
  async addIncidentToQueue(incident) {
    try {
      ...
      // INSTANT ASSIGNMENT: Skip priority window and auto-assign immediately as requested
      this.autoAssignIncident(incident._id);
      
      // Still notify departments for their records
      this.notifyDepartments(assignment);
  ...
  async autoAssignIncident(incidentId) {
    ...
    if (bestDriver) {
      ...
      incident.driverStatus = 'pending_acceptance';      // 🔥 FIX: Set to pending_acceptance to allow Accept/Reject buttons
      await incident.save();
      console.log(`✅ Incident ${incidentId} assigned to driver ${bestDriver.driverName}`);
      
      // Start 2-minute acceptance timer
      try {
        const rejectionTimerService = require('./rejectiontimer');
        const incidentsController = require('../controllers/incidents');
        rejectionTimerService.startTimer(
          incidentId.toString(),
          bestDriver.driverId.toString(),
          bestDriver.department,
          async (iId, dId, dept, reason) => {
            console.log(`🔄 TIMEOUT: Driver ${dId} did not respond to auto-assignment`);
            await incidentsController._performReassignment(
              iId, dId, dept, 'timeout', this.io, bestDriver.driverName, 'Driver did not respond within 2 minutes'
            );
          }
        );
      } catch (timerErr) {
        console.error('❌ Error starting rejection timer for auto-assignment:', timerErr);
      }
      ...
      if (this.io) {
        ...
        // 🔔 FCM: Notify driver via FCM (works even when app is killed)
        try {
          const User = require('../models/User');
          const AlertService = require('./alertService');
          const driverUser = await User.findById(bestDriver.driverId);
          if (driverUser && (driverUser.fcmToken || (driverUser.fcmTokens && driverUser.fcmTokens.length > 0))) {
            await AlertService.sendPushToUser(driverUser, {
              title: '🚨 New Incident Assigned',
              body: `${populatedIncident?.category || 'Emergency'} incident auto-assigned to you at ${populatedIncident?.location?.address || 'your area'}. ETA: ${bestDriver.eta} min.`,
              data: {
                incidentId: incidentId.toString(),
                type: 'driver_assignment'
              }
            });
            console.log(`🔔 FCM sent to auto-assigned driver: ${bestDriver.driverName}`);
          }
        } catch (fcmErr) {
          console.warn('⚠️ FCM auto-assignment driver notification failed:', fcmErr.message);
        }
  ...
  async claimIncident(incidentId, departmentName, driverId) {
    ...
    incident.status = 'assigned';           // Valid from schema
    incident.driverStatus = 'pending_acceptance';      // 🔥 FIX: Set to pending_acceptance
    ...
    try {
      ...
      const driverObj = await require('../models/User').findById(driverId);
      ...
      rejectionTimerService.startTimer(
        ...
        departmentName,
        async (iId, dId, dept, reason) => {
          console.log(`🔄 TIMEOUT: Driver ${dId} did not respond to claimed assignment`);
          await incidentsController._performReassignment(
            iId, dId, dept, 'timeout', this.io, driverObj?.name || 'Unknown', 'Driver did not respond within 2 minutes'
          );
        }
      );
    } catch (timerErr) {
      console.error('❌ Error starting rejection timer for claimed assignment:', timerErr);
    }
