...
io.on('connection', (socket) => {
  ...
  socket.on('join_driver', async (driverId) => {
    ...
    // 🔔 CRITICAL: Immediately push any pending_acceptance incidents 
    // back to the driver in case they missed the socket event (race condition)
    try {
      const mongoose = require('mongoose');
      ...
      try {
        driverObjectId = mongoose.Types.ObjectId.isValid(driverId) 
          ...
          : driverId;
      } catch(e) { driverObjectId = driverId; }

      const pendingIncidents = await Incident.find({
        'assignedTo.driver': driverObjectId,
      ...
      if (pendingIncidents.length > 0) {
        console.log(`📤 Sending ${pendingIncidents.length} pending incident(s) to driver ${driverId} on connect`);
        pendingIncidents.forEach(incident => {
          socket.emit('incidentAssigned', {
            incident: incident,
            eta: null,
            distance: null,
            message: 'You have a pending incident assignment'
          });
        });
      }
    } catch (e) {
      console.error('❌ Error sending pending incidents on driver connect:', e.message);
    }
  });
...
// Relay driver accept/reject between mobile and web sessions
socket.on('driverResponse', (data) => {
  const driverRoom = [...socket.rooms].find((room) => room.startsWith('driver_'));
  if (driverRoom) {
    socket.to(driverRoom).emit('driverResponse', data);
    console.log(`🔄 Relayed driverResponse to ${driverRoom}:`, data?.response);
  }
});

// Relay ephemeral workflow steps (en_route, patient picked up, etc.) between sessions
socket.on('driverWorkflowUpdate', (data) => {
  const driverRoom = [...socket.rooms].find((room) => room.startsWith('driver_'));
  if (driverRoom) {
    socket.to(driverRoom).emit('driverWorkflowUpdate', data);
    console.log(`🔄 Relayed driverWorkflowUpdate to ${driverRoom}:`, data?.status);
  }
});
